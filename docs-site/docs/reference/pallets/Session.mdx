...
The `Staking pallet` uses the Session pallet to get the validator
set.
...
**Pallet #165** · Polkadot SDK<br/>
`pallet-session/1ed66275/stable2409-patch6`

[GitHub](https://github.com/galacticcouncil/polkadot-sdk/tree/1ed662757b03c4f17dc2429c1df84e975d98513a/substrate/frame/session) · [docs.rs](https://docs.rs/crate/pallet-session/38.0.0/source/) · [crates.io](https://crates.io/crates/pallet-session) · [Subscan](https://hydradx.subscan.io/runtime/Session)
...
```rust
Validators: Value<Vec<AccountId32>>
```
...
```rust
QueuedKeys: Value<Vec<(AccountId32, HydradxRuntimeOpaqueSessionKeys)>>
```

### DisabledValidators

Indices of disabled validators. The vec is always kept sorted so that we can find whether a given validator is disabled using binary search. It gets cleared when `on_session_ending` returns a new set of identities.

```rust
DisabledValidators: Value<Vec<u32>>
```
...
```rust
NextKeys: Map<AccountId32 -> {aura: SpConsensusAuraSr25519AppSr25519Public}> [Twox64Concat]
```
...
```rust
KeyOwner: Map<(SpCoreCryptoKeyTypeId, Bytes) -> AccountId32> [Twox64Concat]
```
...
```rust
fn purge_keys(origin: OriginFor<T>)
```
