...
**Pallet #93** · HydraDX<br/>
`pallet-evm-accounts/4ca470c5/master`

[GitHub](https://github.com/galacticcouncil/HydraDX-node/tree/4ca470c5d613d62ce52eb8645770ef5fff263111/pallets/evm-accounts) · [Subscan](https://hydradx.subscan.io/runtime/EVMAccounts)
...
```rust
AccountExtension: Map<H160 -> [u8;12]> [Blake2_128Concat]
```
...
```rust
ContractDeployer: Map<H160 -> Null> [Blake2_128Concat]
```
...
```rust
ApprovedContract: Map<H160 -> Null> [Blake2_128Concat]
```
...
```rust
fn bind_evm_address(origin: OriginFor<T>)
```

### add_contract_deployer

```rust
fn add_contract_deployer(
    origin: OriginFor<T>,
    address: H160,
)
```
...
```rust
fn renounce_contract_deployer(origin: OriginFor<T>)
```
